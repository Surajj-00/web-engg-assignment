...
<template>
  <div class="">
    <header class="flex items-center justify-between lg:justify-around p-4 md:p-6 lg:p-3">
      <img width="45" src="@/assets/images/logo.svg" alt="logo" />

      <nav class="hidden lg:flex items-center justify-between gap-8">
        <RouterLink v-for="link in navLinks" :key="link.name" :to="link.path" class="text-sm font-medium">
          {{ link.name }}
    ...
    <div
      ...
      <!-- links -->
      <RouterLink v-for="link in navLinks" :key="link.name" :to="link.path" @click="toggleMenu" class="text-sm md:text-base">
        {{ link.name }}
        <ChevronDown class="w-3 h-3 inline" />
      </RouterLink>
